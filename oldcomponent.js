class Componentt {
  constructor(type = OR, pos = createVector(500, 200), n1 = 2, n2 = 1) {
    ...
    if (this.type === NODE) {
      ...
      this.input = [];
      this.output = [];
      this.pos = createVector(width * 0.12, height * 0.1);
      this.h = height * 0.8;
      this.w = width * 0.83;
  ...
  draw() {
    if (this.type === NODE) {
      ...
      if (this.source && this.source.exists && this.showWire) {
        let x1 = this.pos.x;
        let y1 = this.pos.y;
      ...
      line(this.pos.x + 50, this.pos.y, this.pos.x + 50, this.pos.y + this.h);
      line(
        this.pos.x + this.w - 50,
        this.pos.y,
        this.pos.x + this.w - 50,
        this.pos.y + this.h
      );
  ...
  updateStructure() {
    if (this.type === NODE) {
      if (mode === IC && !this.static) {
        let x1 = chipObject.pos.x;
        let y1 = chipObject.pos.y;
        let hh = chipObject.h;
        let ww = chipObject.w;
        if (!this.pushedIntoChipOutput && !this.pushedIntoChipInput) {
        }
        if (
          this.pos.x > x1 &&
          this.pos.x < x1 + 50 &&
          ...
          if (!this.pushedIntoChipInput) chipObject.input.push(this);
          this.pushedIntoChipInput = true;
          this.pushedIntoChipOutput = false;
          let index = gates.indexOf(this); // Find the index of this node object
          if (index !== -1) {
            gates.splice(index, 1); // Remove 1 element at the found index
          }
          index = chipObject.output.indexOf(this); // Find the index of this node object
          if (index !== -1) {
            chipObject.output.splice(index, 1); // Remove 1 element at the found index
          }
        } else if (
          this.pos.x > x1 + ww - 50 &&
          this.pos.x < x1 + ww &&
          ...
          if (!this.pushedIntoChipOutput) chipObject.output.push(this);
          this.pushedIntoChipOutput = true;
          this.pushedIntoChipInput = false;
          let index = gates.indexOf(this); // Find the index of this node object
          if (index !== -1) {
            gates.splice(index, 1); // Remove 1 element at the found index
          }
          index = chipObject.input.indexOf(this); // Find the index of this node object
          if (index !== -1) {
            chipObject.input.splice(index, 1); // Remove 1 element at the found index
          }
        } else {
          this.pushedIntoChipOutput = false;
          this.pushedIntoChipInput = false;
